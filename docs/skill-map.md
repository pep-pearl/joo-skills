...
| --- | --- | --- |
| `repo-indexing` | 새 프로젝트를 AI 친화적으로 인덱싱할 때 | `AI_INDEX.md` router, `.ai/indexing/maps/*`, rules, header candidates |
| `repo-navigation` | 작업 시작 전 최소 파일을 고를 때 | intent classification, read plan, one-shard read list, uncertainty |
| `ai-metadata-maintenance` | route/domain/API/state/package 변경 후 metadata 갱신 판단 | maintenance summary for router/maps/headers |
