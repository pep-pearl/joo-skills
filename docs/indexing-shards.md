...
Use profile budgets rather than fixed line counts:

- `AI_INDEX.md`: router-only and typically a few KB
- total navigation artifacts: bounded by source ratio plus profile min/max
- shard count and domain-shard count: profile-capped
- each shard: byte, token, and entry capped
- file-map: separate byte pool
- each file entry: one compact line

When capacity is exceeded, evict low priority-per-byte entries and omit low-utility shards. Do not expand the cap silently.

## Adaptive levels

Shards are not generated by default for every repository. Level 0 emits assessment only, Level 1 emits a router candidate, Level 2 emits bounded core/selected-domain shards, and Level 3 adds file-map lookup support. Budget profiles control entry, shard, token, and byte caps. `auto` stays conservative without ROI evidence; `force` changes the indexing level for experiments but does not disable budget enforcement.

## Budgeted selection and eviction

Line-count caps are secondary. The scanner now applies byte and file-count budgets through `tight`, `balanced`, or `retentive` profiles.

- emit only selected shards; empty/low-utility shards are omitted
- reserve fixed Markdown overhead before allocating entry bytes
- select entries by deterministic priority and priority-per-byte density
- preserve recent-error and minimum-residence entries when capacity permits
- limit refresh churn with a maximum replacement ratio
- penalize legacy/archive/example/generated/test/story candidates
- keep file-map JSON flat and byte-capped
- never link maintenance reports or state files from the runtime router

When a new entry is more useful than an existing entry and capacity is full, remove the lowest unprotected priority-density entry. Do not append indefinitely.

`AI_INDEX.md` is router-only regardless of profile. A larger `retentive` profile increases candidate retention, not the number of shards a runtime agent may read.
